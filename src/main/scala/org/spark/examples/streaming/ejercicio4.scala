...
object ejercicio4 {
  ...
  def main(args: Array[String]): Unit = {
   ...
   //Voy a usar el de las autorizaciones. hago un print para que me de los tipos de lineas
    ...
    val  = numberEvents_authRDD.foreachRDD(rdds=>{
      val filtrado = rdds.filter(aut=>aut.Message.contains("failed"))
      filtrado.map(x=>(x.Source,1)).reduceByKey((acum,nuevo)=>acum+nuevo)
    }).print
...
/*  Given the previous stream obtained in Exercise 3, filter those hosts that are already blacklisted in the system.
    ...
    As the output of the stream, maintain the same structure
    *(host_i, numberRequests)*. Print the results.
*/
